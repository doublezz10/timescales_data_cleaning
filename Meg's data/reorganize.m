    ...
    if strcmp(brain_area,'SC') == 1
        ...
        for trial = 1:length(units(unit).fixOn)
            ...
            for spike = 1:length(spikes)
                if spike >= fixation_start && spike <= fixation_end
                    fixation_spikes(end+1) = spike;
                end
                
                if spike >= task_start && spike <= task_end
                    trial_spikes(end+1) = spike;
                end
