...
task = cell(2,length(timeseries));
rest = cell(2,length(timeseries));
    ...
    for trial=1:length(trial_start{session})
       ...
       for spike=1:length(unit_spikes)
           ...
           if unit_spikes(spike) >= trialstart && unit_spikes(spike) <= trialstart + odor_on
               ...
               rest_spikes = [rest_spikes unit_spikes(spike)];
               rest_trials = [rest_trials trial];
           
           elseif unit_spikes(spike) >= trialstart + odor_on && unit_spikes(spike) <= trialstart + water_off
               
               task_spikes = [task_spikes unit_spikes(spike)];
               task_trials = [task_trials trial];
          
            end
        end
    end
    
    rest{1,unit} = rest_spikes; 
    rest{2,unit} = rest_trials;
    task{1,unit} = task_spikes;
    task{2,unit} = task_trials;
